...
add_action( 'wp_enqueue_scripts', 'wooinfluencer_public_assets' );
function wooinfluencer_public_assets()
{
	wp_enqueue_style( 'woocommerce_stylesheet', plugins_url('style.css', __FILE__) );
}
...
if( get_option('display_sales_shop_page', false) == true) {
	add_action( 'woocommerce_before_shop_loop_item_title', 'total_sales_shop_page', 11 );
}

function total_sales_single_product_page(){
	echo  nl2br(do_shortcode(get_option('show_in_product_page_view_editor')));
}

function total_sales_shop_page(){
	echo  '<div class="wooinfluencer">' . nl2br(do_shortcode(get_option('show_in_product_in_list_view_editor'))) . '</div>';
}
...
function get_total_sales($atts)
{
  ...
  $product 	  	= $_pf->get_product($id);
	...
	if( $total_sales == 0 ) {
		return nl2br('<span '. $animation .'>' .$total_sales. '</span>');
	} else if( $total_sales == 1) {
...
add_action( 'admin_init', 'wsp_register_admin_settings' );
function wsp_register_admin_settings() { 

	register_setting( 'wsp-settings-group', 'display_sales_single_product_page' );
	register_setting( 'wsp-settings-group', 'display_sales_shop_page' );
	register_setting( 'wsp-settings-group', 'show_in_product_in_list_view_editor' );
	register_setting( 'wsp-settings-group', 'show_in_product_page_view_editor' );
}
...
*/
function settings_page_ui() { ?>

<div class="wsp-wrap">
  <div class="wsp-settings-header">
  	...
  	<div class="accordion">
    	...
    	<div class="panel-content">
				<div style="display: block;"><br>
				  <p style="padding:10px 20px"><strong class="wps-strong">Stats available</strong></p>
				  ...
				  <p style="padding:10px 20px"><strong class="wps-strong">Locations</strong></p>
				  <table class="toggle-table">
				    <tbody>
				      <tr>
				        <td>Free version</td>
				  ...
				  <p style="padding:10px 20px"><strong class="wps-strong">Variables</strong></p>
				  <table class="toggle-table">
				    <tbody>
				    <tr>
				      <td>Free version</td>
				  ...
				  <p style="padding:10px 20px"><strong class="wps-strong">Examples</strong></p>
				  <table class="toggle-table">
				    <tbody>
				      <tr>
				        <td>Free version</td>
  					...
  					<a class="button wsp-button button-primary" target="_blank" href="https://shopitpress.com/plugins/woocommerce/wooinfluencer/?utm_source=wordpress.org&utm_medium=backend-usage&utm_content=v1.0.0&utm_campaign=wooinfluencer"><span>Get Pro version</span></a>
     			</p>
     		</div>
  		</div>

  <form method="post" action="options.php">
  <?php settings_fields( 'wsp-settings-group' ); ?>
